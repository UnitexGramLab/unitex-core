...
Grf* tfst_transitions_to_grf_states(Tfst*,int*,int,int,int*,char*,int);
 ...
 */
Grf* sentence_to_grf(Tfst* tfst,char* font,int fontsize) {
...
Grf* grf=tfst_transitions_to_grf_states(tfst,rank,maximum_rank,width_max,pos_X,font,fontsize);
free(rank);
free(pos_X);
return grf;
}
