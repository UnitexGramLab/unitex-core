...
void produce_mwu_entries(U_FILE* grf,int n_parts,struct dela_entry** entries,MultiFlex_ctx* ctx,
                         Alphabet* alphabet,Korean* korean,Jamo2Syl* jamo2syl,
...
while ((size_line=u_fgets(line,MAX_LINE_SIZE,delas))!=EOF) {
   ...
   if (OK) {
       ...
       * component */
      produce_mwu_entries(grf,n_parts,entries,ctx,alphabet,korean,jamo2syl,morpho,
...
for (int i=0;i<n_parts-1;i++) {
    ...
    * dictionary lookup */
   convert_Korean_text(entries[i]->inflected,inflected_jamo,korean,alphabet);
...
for (int i = 0; i < forms.no_forms; i++,(*line)++) {
   (*current_state)++;
   convert_Korean_text(forms.forms[i].form,inflected_jamo,korean,alphabet);
