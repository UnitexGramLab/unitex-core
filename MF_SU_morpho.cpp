...
int SU_inflect(SU_id_T* SU_id, f_morpho_T* desired_features, SU_forms_T* forms,
   ...
   err = SU_explore_state(inflected, SU_id->lemma->unit, inflection_codes,
         fst2[T], 0, desired_features, forms, semitic ? semitic_ : NULL, 0, 0,
         NULL, local_sem_code);
   return err;
}
...
int SU_inflect(unichar* lemma, char* inflection_code, unichar** filters,
   ...
   err=SU_explore_state(inflected, lemma, inflection_codes, fst2[T], 0, NULL,
         forms, semitic ? semitic_ : NULL, 0, 0, filters, local_semantic_code);
   return err;
}
...
int SU_explore_state(unichar* flechi, unichar* canonique, unichar* sortie,
      ...
      unsigned int var_in_use, unichar **filters, unichar *local_semantic_codes) {
   int err;
      ...
      if (desired_features!=NULL) {
         ...
         while (feat[f]) {
            ...
            if (SU_feature_agreement(feat[f], desired_features)) {
               ...
               forms->forms=(SU_f_T*)realloc(forms->forms, (forms->no_forms+1)
                     *sizeof(SU_f_T));
               if (!forms->forms) {
   ...
   while (t!=NULL) {//u_fprintf(stderr,"Explore 1\n");
      retour_tag = SU_explore_tag(t, flechi, canonique, sortie, a, NULL,
            desired_features, forms, semitic, flag_var, var_in_use, filters,
            local_semantic_codes);
      retour_all_tags += retour_tag;
      t=t->next;
   }
   if (default_trans != NULL) {//u_fprintf(stderr,"PASS02\n");retour_tag = SU_explore_tag(default_trans,flechi,canonique,sortie,a,NULL,desired_features,forms,semitic,flag_var,var_in_use,filters);
...
struct inflect_infos* new_inflect_infos() {
   struct inflect_infos* i=
         (struct inflect_infos*)malloc(sizeof(struct inflect_infos));
   if (i==NULL) {
...
int SU_explore_tag(Transition* T, unichar* inflected, unichar* lemma,
   ...
   if (T->tag_number < 0) {
      ...
      SU_explore_state_recursion(inflected, lemma, output, a,
            a->initial_states[-(T->tag_number)], &L, desired_features, forms, semitic,
            flag_var, var_in_use, filters, local_semantic_codes);
      while (L!=NULL) {
         if (LIST==NULL) {//u_fprintf(stderr,"Explore state 1\n");
            retour_state = SU_explore_state(L->inflected, lemma, L->output, a,
                  T->state_number, desired_features, forms, L->semitic,
                  flag_var, var_in_use, filters, local_semantic_codes);
            ...
            retour_state = SU_explore_state_recursion(L->inflected, lemma,
                  L->output, a, T->state_number, LIST, desired_features, forms,
   ...
   if (u_strcmp(tag, "<E>")) {
      ...
      for (int pos_tag=0; tag[pos_tag]!='\0';) {
         switch (tag[pos_tag]) {
         case '<':
            retour = flex_op_with_var(Variables_op, stack, tag, &pos, &pos_tag,
                  &var_in_use);
   ...
   if (retour) {
      if (LIST==NULL) {//u_fprintf(stderr,"Explore state 2\n");
         retour_state= SU_explore_state(stack, lemma, out, a, T->state_number,
               desired_features, forms, semitic ? semitic2 : NULL, flag_var,
               var_in_use, filters, local_semantic_codes);
