...
int u_member(unichar c, unichar *str, int *where) {
...
for (i=0; i<u_strlen(str1); i++)	where[i] = 0;
for (i=u_strlen(str1); i<MAX_STR_LEN; i++) where[i] = -1;
...
for (i=0; i<len; i++) 
	...
	if ((w = u_member(str2[i],str1,where)) == -1) return(0);
...
void u_tri_a(void *T, int *cnt, int len)
{
  ...
  for (i=0, j=1; i<*cnt; i++)
	...
	{
	  i++;
	  if (i != j) u_strcpy(TAB + i*len,TAB + j*len);
	  j++;
	}
    }
}
