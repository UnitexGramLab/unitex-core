...
#
# Inspired _and mainly adapted_ from boost's pretty printers
# All credits for this file to Rüdiger Sonderfeld and Matei David
# Boost Software License - Version 1.0 - August 17th, 2003
#
# @source https://github.com/ruediger/Boost-Pretty-Printer
# @source https://github.com/bloomberg/bde-tools
#
...
def message(s):
  ...
  """
  print('*** unitex printers: ' + s, file=sys.stderr)
...
def call_static_method(t, f, *args):
  ...
  try:
    ...
    long_message(
      ...
      '\t  py unitex.static_method[("' + str(t.strip_typedefs())
      + '", "' + f + '")] = <f>')
    raise gdb.error
...
def get_raw_ptr(p):
  ...
  try:
    ...
    long_message(
      ...
      '\t  py unitex.raw_ptr["' + str(p.type.strip_typedefs()) + '"] = <f>')
    raise gdb.error
...
def is_null(p):
  ...
  long_message(
    'is_null',
    '\n\tto bypass this with python function <f>, add:\n' +
    '\t  py unitex.null_dict["' + str(p.type.strip_typedefs()) + '"] = <f>')
  raise gdb.error
...
type_printer_list = []
unitex_printer_list = []
unitex_regex_printer_list = []
trivial_printer_list = []


def register_printers(obj=None, unitex_version=None):
  """
  Register top-level printers 'unitex' and 'trivial' with objfile `obj`.
  """
  if unitex_version is None:
    message('No unitex version was provided')
    message('Use e.g.: unitex.register_printers(unitex_version=(4,0,0))')
  else:
    # printers
    supported_printers = [printer for printer in unitex_printer_list
              if printer.min_supported_version <= unitex_version <= printer.max_supported_version]
    if supported_printers:
      unitex_printer_gen = Printer_Gen('unitex')
      for printer in supported_printers:
        unitex_printer_gen.add(printer)
      gdb.printing.register_pretty_printer(obj, unitex_printer_gen, replace=True)
    else:
      message('No unitex printers are available for unitex version {}.{}.{}!'.format(*unitex_version))

    # regex_printers
    supported_regex_printers = [regex_printer for regex_printer in unitex_regex_printer_list
              if regex_printer.min_supported_version <= unitex_version <= regex_printer.max_supported_version]
    if supported_regex_printers:
      unitex_regex_printer_gen = gdb.printing.RegexpCollectionPrettyPrinter('unitex_regex')
      for regex_printer in supported_regex_printers:
        unitex_regex_printer_gen.add_printer(regex_printer.printer_name, regex_printer.template_name, regex_printer)
      gdb.printing.register_pretty_printer(obj, unitex_regex_printer_gen)
    else:
      message('No unitex regex_printers are available for unitex version {}.{}.{}!'.format(*unitex_version))
...
def add_printer(p):
  """
  Decorator that adds the given printer `p` to the top-level 'unitex' printer.
  """
  unitex_printer_list.append(p)
  return p

def add_regex_printer(p):
  """
  Decorator that adds the given printer `p` to the top-level 'unitex' printer.
  """
  unitex_regex_printer_list.append(p)
  return p
...
def cond_add_printer(cond, msg):
  """
  Decorator-generator that conditionally adds a printer to the top-level 'unitex' printer.
  """
  if cond:
    return add_printer
  else:
...
def add_trivial_printer(tn, f):
  ...
  py unitex.add_trivial_printer("List_Obj", lambda v: v['_val'])
...
# @source https://github.com/bloomberg/bde-tools (bde_printer.py)
...
# @source https://github.com/ruediger/Boost-Pretty-Printer (wave_1_71.py)
...
# @source https://github.com/bloomberg/bde-tools (bde_printer.py)
def string_address(data):
    char_ptr_type = gdb.lookup_type('unitex::unichar').const().pointer()
    c_str = data.cast(char_ptr_type)
    return '0x%x ' % c_str if options['print_address'] else ''
...
def char_make(ci):
  ...
  if cc in string.printable:
    ...
    print_chr = "\{0:03o}".format(int(ci))
  return print_chr

# c_str[i] as char 
def char_make_at(c_str, i):
  ci = (c_str + i).dereference()
  return char_make(ci)

# @see https://github.com/bloomberg/bde-tools (bde_printer.py)
def string_make(data, length):
  print_len = options['print_elements']
  ...
  for i in range(print_len):
    print_str += char_make_at(c_str, i)
...
# (gdb) python import unitex.printers
# (gdb) python unitex.multi_index_selector[0x7fffffffd770] = 1
# (gdb) p s_5
#
...
#
options = {'hide_intrusive_hooks': True, 'print_address': True, 'print_elements': 128}

# Latest unitex currently supported by printers
last_supported_unitex_version = (4, 1, 0)
