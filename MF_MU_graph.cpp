...
int MU_graph_explore_state(Fst2State q, MU_forms_T* forms) {
  ...
  if (q->control & 1) {
    ...
    if (!forms->forms) {
      error("Memory allocation problem in function 'MU_graph_explore_state'!\n");
      return 1;
    }
    MU_f_T* f;
    f = &(forms->forms[0]);
  ...
  while (t) {
    ...
    if (!l) {
      fprintf(stderr,"Memory allocation problem in function 'MU_graph_explore_state'!\n");
      return 1;
    }
...
int MU_graph_explore_label(MU_graph_label_T* l,Fst2State q_bis, MU_forms_T* forms) { 
  ...
  //Explore the label's input (and recursively also the output)
	...
	if (!tmp) {
	  fprintf(stderr,"Memory allocation problem in function 'MU_graph_explore_label'!\n");
	  return 1;
	}
...
int MU_graph_explore_label_in_var_rec(SU_id_T* u, MU_graph_morpho_T* l_in_morpho, int i_morpho, f_morpho_T* feat, MU_graph_morpho_T* l_out_morpho, Fst2State q_bis, MU_forms_T* forms) {
  ...
  if (!l_in_morpho) {
    ...
    for (mf=0; mf<forms->no_forms;mf++) {
      ...
      if (!tmp) {
	fprintf(stderr,"Memory allocation problem in function 'MU_graph_explore_label_in_var_rec'!\n");
	return 1;
      }
      u_strcpy(tmp,u->form);
...
int MU_graph_explore_label_in_morph_unif(MU_graph_category_T* c, SU_id_T* u, MU_graph_morpho_T* l_in_morpho, int i_morpho, f_morpho_T* feat, MU_graph_morpho_T* l_out_morpho, Fst2State q_bis, MU_forms_T* forms) {
  ...
  if (unif_instantiated(var)) {
	...
	forms->forms = (MU_f_T*)realloc(forms->forms,sizeof(MU_f_T)*(forms->no_forms+forms_bis.no_forms));
	if (! forms->forms) {
	  fprintf(stderr,"Memory allocation problem in function 'MU_graph_explore_label_in_morph_unif'!\n");
	  return 1;
	}
	for (mf=0; mf<forms_bis.no_forms; mf++) 
	  forms->forms[forms->no_forms+mf] = forms_bis.forms[mf];
...
int MU_graph_explore_label_out_morph_unif(MU_graph_category_T* c, MU_graph_morpho_T* l_out_morpho, int i_morpho, f_morpho_T* feat, Fst2State q_bis, MU_forms_T* forms) {
  ...
  if (unif_instantiated(var)) {
	...
	forms->forms = (MU_f_T*)realloc(forms->forms,sizeof(MU_f_T)*(forms->no_forms+forms_bis.no_forms));
	if (! forms->forms) {
	  fprintf(stderr,"Memory allocation problem in function 'MU_graph_explore_label_out_morph_unif'!\n");
	  return 1;
	}
	for (mf=0; mf<forms_bis.no_forms; mf++) 
	  forms->forms[forms->no_forms+mf] = forms_bis.forms[mf];
...
int MU_graph_scan_label_in(unichar* label, MU_graph_in_T* MU_label_in) {
  ...
  if (*pos != (unichar)'<') {
    ...
    if (*pos != (unichar)'$') {
      fprintf(stderr,"In graph %s label format incorrect in ",MU_lemma->paradigm);
      u_fprints(label,stderr);
      fprintf(stderr," (at position %d): ",(int)(pos-label));
      fprintf(stderr," a '$' missing after '<'.\n");
      return 1;      
    }
    pos++;  //Omit the '$'
    ...
    if (!l) {
      ...
      u_fprints(label,stderr);
      fprintf(stderr," (at position %d): ",(int)(pos-label));
      fprintf(stderr," unit number missing after \'$\'.\n");
      return 1;
    }
    ...
    if ((*pos != (unichar) ':') && (*pos != (unichar) '>')) {
      fprintf(stderr,"In graph %s label format incorrect in ",MU_lemma->paradigm);
      fprintf(stderr,"Graph label format incorrect in ");
      u_fprints(label,stderr);
      fprintf(stderr," (at position %d): ",(int)(pos-label));
      fprintf(stderr,"':' or '>' missing.\n");
      return 1;
    }
  ...
  if (*pos != 0) {
    ...
    u_fprints(label,stderr);
    fprintf(stderr," (at position %d): ",(int)(pos-label));
    fprintf(stderr," end of label expected.\n");
    return 1;      
  }
  return err;
}
...
int MU_graph_scan_label_out(unichar* label, MU_graph_out_T* MU_label_out) {
  ...
  if (!err)
	...
	fprintf(stderr,"In graph %s label format incorrect in ",MU_lemma->paradigm);
	u_fprints(label,stderr);
	fprintf(stderr,": an output label may not contain a double assignment \'==\'.\n");
	return 1;
      }
  //Closing '>'
  if (*pos != (unichar) '>') {
...
int MU_graph_scan_graph_morpho(unichar* label, MU_graph_morpho_T* MU_graph_morpho) {
  ...
  while (!done) {
    ...
    if (!cat) { 
      ...
      fprintf(stderr," is not a valid category\n");
      return 1;
    }
	...
	u_fprints(label,stderr);
	fprintf(stderr," (at position %d): ",(int)(pos-label));
	fprintf(stderr,"a variable missing after \'$\'.\n");
	return 1;      
      }
	...
	fprintf(stderr," (at position %d): ",(int)(pos-label));
	u_fprints(tmp,stderr);
	fprintf(stderr,"a value missing after \'=\'.\n");
	return 1;      
      }
	...
	fprintf(stderr," (at position %d): ",(int)(pos-label));
	u_fprints(tmp,stderr);
	fprintf(stderr," is not a valid value in the domain of ");
