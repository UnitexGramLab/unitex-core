...
int main_Seq2Grf(int argc,char* const argv[]) {
	...
	U_FILE* grf=u_fopen_creating_unitex_text_format(encoding_output,bom_output,output/*text_grf*/,U_WRITE);
	if (grf==NULL){
		u_fclose(f);
...
///##########
	...
	Tfst* tfstFile=open_text_automaton(text_tfst);
	load_sentence(tfstFile,1);
	if (tfstFile == NULL )u_printf("tfstFile==NULL !!! \n");
	if (tfstFile->automaton==NULL) u_printf("tfstFile->automaton==NULL !!! \n");
	if (tfstFile == NULL) u_printf("tfst NULL\n");
	u_printf("test 6 \n");
	Grf* grfObj=sentence_to_grf(tfstFile,fontname,size,is_sequence_automaton);
...
void build_sequences_automaton(U_FILE* f,
			...
			if (buffer[i]==tokens->SENTENCE_MARKER){
				...
				if(buffer[i]!=tokens->SPACE){
					...
					if (i==0){
						...
						add_outgoing_transition(tfst->automaton->states[0],				tag_number,current_state+1);
						...
						//				add_outgoing_transition(tfst->automaton->states[current_state],	tag_number,current_state+1);
						add_outgoing_transition(tfst->automaton->states[current_state],	get_value_index(tmp_states->str,tmp_tags),current_state+1);
						trans=tfst->automaton->states[current_state]->outgoing_transitions;
						if (trans==NULL){
	...
	for (i=0;i<nbsentence;i++){
		add_outgoing_transition(tfst->automaton->states[last_transitions[i][0]],last_transitions[i][1],current_state);
	}
