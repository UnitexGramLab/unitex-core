...
const char* usage_Locate =
         ...
         "  -E X/--elg_scripts_path=X: uses ELG scripts directory X instead of App/Scripts\n"
...
const char* optstring_Locate=":t:a:m:SLAIMRXYZln:d:E:cewsxbzpKVhk:q:o:u:g:Tv:$:@:C:P:HQN+:";
const struct option_TS lopts_Locate[]= {
  ...
  {"elg_scripts_path",required_argument_TS,NULL,'E'},
...
MsgLogger& logger = MsgLogger::get(); //uncomment.this
...
logger.params.message_details  = MsgLogMessageDetails::kFlagFullDetails;
Chrono chrono_start = Chrono::now();
...
char elg_scripts_path[FILENAME_MAX]="";
...
while (EOF!=(val=options.parse_long(argc,argv,optstring_Locate,lopts_Locate,&index))) {
   ...
   case 'd': if (options.vars()->optarg[0]=='\0') {
             ...
             break;
   case 'E': if (options.vars()->optarg[0]=='\0') {
                error("You must specify a non empty Scripts path\n");
                free_vector_ptr(injected_vars,free);
                free_locate_trace_param(list_param_trace);
                free(morpho_dic);
                return USAGE_ERROR_CODE;
             }
             strcpy(elg_scripts_path,options.vars()->optarg);
             break;
...
// if the path of the ELG Scripts is not given,
// a default path is calculated
if (elg_scripts_path[0] == '\0') {
  ...
  // Copy back
  strcpy(elg_scripts_path, exec_path);
}
...
Chrono chrono_end = Chrono::now();

//u_printf("==%S== (%s %s)%s [%S | %-12.0f %s:%0.4d] %s\n",
                          //logger.thread_name().c_unichar(),
                          //SeverityLevel::Info.alias(),
                          //SeverityLevel::Info.name(),
                          //"     ",
                          //time.as_timestamp().c_unichar(),
                          //chrono_start.elapsed(time).as_microseconds(),
                          //"s",
                          //UNITEX_FILE_LINE,
                          //"Working..."
                          //);

u_printf("Time elapsed: %.2fs\n", chrono_start.elapsed(chrono_end).as_seconds());
return (!OK);
}
