...
\author{B.Burak Arslan \\ $<$burak\@.arslan@arskom.\@com\@.tr$>$}
...
\frame{
	...
	\item An active research area even before the computing era.
...
\frame{
	...
	\item ``Recurrent combinations of words that co-occur more often than expected by chance and that correspond to arbitrary word usages''	\cite{smadja93}
		\begin{itemize}
			\item later research showed that collocations can't be located by purely statistical methods\cite{1118854}.
		\end{itemize}
...
\subsection{Overview}
\frame{
	\frametitle{ Collocation Extraction Systems: Overview I }
	\begin{itemize}
...
\frame{
	\frametitle{ Collocation Extraction Systems: Overview II }
	\begin{itemize}
...
\frame{
	...
	\begin{itemize}
		...
		\item This is where the memory needs may be more than typical. Two precautions:
		\begin{enumerate}
			\item \textbf{compacting}
			\item Dictionary levels
		\end{enumerate}
	\end{itemize}
}
\frame{
...
\frame{
	...
	\small The array that stores this information takes exactly $456.797.420$ bytes in entry level 0
	\end{center}
}
...
\frame{
	...
	\begin{itemize}
		...
		\item Once this data structure is extended, collocations can be combined by studying the correlation of occurence between two pairs that share a common word.
	\end{itemize}
}
\frame{
